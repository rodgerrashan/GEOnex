...
export default function RegisteredDevices() {
  ...
  const fetchUserDevices = async () => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/user/${userId}/registereddevices`
      );
      if (!response.ok) {
  ...
  return (
    <>
      <div className="flex flex-col gap-6 w-full t-gray-900 dark:text-gray-100">
        ...
        <div className="overflow-x-auto w-full">
          <div className="min-w-[600px]">
            ...
            bg-gray-100 dark:bg-gray-700 p-3 rounded-lg min-w-[600px]">
              <span className="flex-shrink-0 w-[25%] text-sm md:text-base font-semibold">
                Device Name
              </span>
              <span className="flex-shrink-0 w-[20%] text-sm md:text-base font-semibold text-center">
                Type
              </span>
              <span className="flex-shrink-0 w-[20%] text-sm md:text-base font-semibold text-center">
                Device Code
              </span>
              <span className="flex-shrink-0 w-[25%] text-sm md:text-base font-semibold text-center">
                Registered Date
              </span>
            </div>
            ...
            {loading ? (
              ...
              <p className="text-gray-600 dark:text-gray-400 py-4 text-center">
                You have no devices registered.
              </p>
            ) : (
              devices.map((device) => (
                <div
